...
function updatetotal() {
    ...
    for (let i = 0; i < cartBoxes.length; i++) {
        ...
        if (priceElement && quantityElement) {
            let price = parseInt(priceElement.innerText.replace("TH", ""));
            let quantity = quantityElement.value;
            total = total + (price * quantity);
        }
    }

    let totalComma = numberWithCommas(total);
    document.getElementsByClassName("total-price")[0].innerText = totalComma + "TH";
}
